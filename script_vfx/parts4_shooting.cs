...
public class parts4_shooting : MonoBehaviour
{
    float shooting_t = 0f;
    float delay_t = 0f;
    public GameObject parts4_bullet;
            ...
            if (player_script.is_spawn)
            {
                ...
                if (shooting_t > 2.5f)
                {
                    if (delay_t > 0.2f)
                    {
                        if (count < 2)//총알 발사 횟수
                        {
                            Instantiate(parts4_bullet, fire_pos.position, Quaternion.identity);
                            count++;
                        }
                        else
                        {
